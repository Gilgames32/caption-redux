...
def gif_from_video(in_vid: str, out_gif: str):
    ...
    st = time.time()
    ffmpeg.input(in_vid).filter('palettegen', reserve_transparent=1).output(__palfne).run(quiet=True)
    logging.debug(f"Generated palette")
    
    ffmpeg.filter([ffmpeg.input(in_vid), ffmpeg.input(__palfne)], 'paletteuse', alpha_threshold=128).output(out_gif).run(quiet=True)
    et = time.time()
